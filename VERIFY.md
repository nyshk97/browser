...
- **`[verify] verify は Nemo が起動していると実行できない` で止まり、pid の `userData` が別セッションの scratchpad
  （`/private/tmp/claude-501/…/<別の session id>/scratchpad/ud`）なら、他の Claude Code セッションが起動した検証インスタンス**。
  `ps -o ppid,etime -p <pid>` で親が launchd（ppid 1）なら起動元のスクリプトは終わっていて残骸の可能性が高いが、
  自分が起動したものではないので勝手に止めずユーザーに聞く。マーカー（`.nemo-run/<pid>.json`）はプロセスが消えていれば
  ハーネスが自分で掃除する
...
- **2本指スワイプで戻る / 進む**（判定はページから見えない隔離ワールドに入っていること・
  ...
  **DOCTYPE の無い（quirks mode）文書の上でスクロールしても固まらないこと**）
...
- **リンクのクリックで開いた直後の Peek にキーフォーカスがある**こと（`splitDiagnostics().focusedWebContentsId`。
  null だと実キーの Esc が responder chain に落ちてフルスクリーンが解ける）/ Esc で閉じること /
  **閉じたら親のページにフォーカスが戻る**こと
...
- UI 操作（合成イベント）:
  ...
  - **閉じている Favorite のセルをダブルクリックしても編集に入らず、タブがちょうど 1 つ増える**
    （グリッドの導線は右クリックだけ。`openFavorite` が冪等なので 2 発目は選ぶだけ）/
    **閉じている Favorite の単クリックは遅延しない**（クリック直後から 30ms 刻みでタブ増加を待ち、
    経過時間が 250ms 未満であること。`settle()` 待ちだと旧実装でも PASS するので経過時間で見る）/
    Favorite は右クリックの「名前を変更」で編集に入り、定義に反映される
...
await window.nemo.suggest('github nyshk97 mobil') // 空白区切りは全語 AND（順序不問）
...
4. **ダブルクリックでのリネーム**（ピン留め行・一時タブ行。実マウスでの手触り・**IME の変換確定 Enter で編集が閉じないこと**。
   合成イベントでは `isComposing` の経路を通せない。**フォルダと Favorites は対象外**（右クリックの「名前を変更」から。
   Favorites の枠はダブルクリックしても開くだけ））
...
6. **トラックパッドの2本指スワイプ**（自走検証は合成イベントなので、実際の指では別途見る）
   - 指を右へ払って戻る / 左へ払って進む
   - **戻った勢い（慣性）でもう1ページ戻らないこと**
7. **beforeunload の離脱確認**（`will-prevent-unload` → `showMessageBoxSync` は**ネイティブ modal**で
   CDP から押せない。自走検証は `NEMO_VERIFY_UNLOAD_CHOICE=leave` の自動選択で「リロードが通ること」
   しか見ていないので、ダイアログそのものはここを触ったら目視する）
   - `http://127.0.0.1:8787/login.html?beforeunload=1` を開き、**ページを一度クリックしてから** ⌘R
     （クリック＝user activation が無いと Chromium が beforeunload を無視してダイアログ自体が出ない）
   - 「サイトを離れますか？」が出る → 「キャンセル」でリロードされない
   - もう一度 ⌘R → 「このページを離れる」でリロードされる
...
オーバーレイの開閉」まで。残りは手で見る。

1. **ライブラリ（⌘Y）の見た目**。履歴とアーカイブの切り替え、行のダブルクリックで開けること、
   × で1件消せること、🗑 で全消しできること
2. **設定画面（⌘,）**。Chrome 拡張 / GitHub の PR / Basic 認証 / Basic 認証の引き継ぎ / ブックマークの引き継ぎ / データ の
   6 節だけが出ること（タブの sleep・起動と検索・キーバインド・既定ブラウザは画面に出さない。
   値は `settings.json` を直接編集する）
3. **シークレットウィンドウ（⌘⇧N）**。サイドバーが紫になり、注意書きが出て、
   **拡張のアイコンが出ない**こと。
   - **閉じたら消えること**（自走検証はウィンドウを閉じられないので、ここは人が見る）。
     シークレット窓を**全部閉じて**から ⌘⇧N で開き直し、次を確かめる:
...
4. **既定ブラウザ**（パッケージ版でのみ）:
   ...
   # システム設定 → デスクトップとDock → デフォルトのWebブラウザ の候補に「Nemo Dev」が出ること
   # （`electron-builder.yml` の `protocols` が Info.plist に反映されている証拠）
...
5. **一時タブの自動アーカイブ**を実運用の設定（24 時間）で確認するのは現実的でないので、
   `settings.json` の `tabArchiveHours` を 1 などに落として翌日見る。**ピン留めしたタブが消えていないこと**を必ず見る
...
- 小窓のマイク・カメラは**表示だけで押せない**（押してもページ側のクリック数が増えず、
  `<button>` でも `no-drag` でもない。同じ式で「会議へ移動」が `<button>` かつ `no-drag` と
  取れることを陽性コントロールにする）。ページ側でミュート / カメラ OFF にすると小窓の表示が追従し、戻すと ON に戻る
- **背面のまま**ページ側で切っても表示が追従する（Chromium の rAF スロットリングを外している。
  偽 Meet の属性書き換えは本物と同じく rAF 越しなので、外し忘れるとここで落ちる）
...
3. **⌘H すると小窓も一緒に隠れる**（R1 の結論。Electron に `canHide` 相当の API が無い）。
   ⌘Tab で戻せば再び出る
4. 小窓のマイク・カメラの表示が **Meet 側の状態と一致する**こと（反転していないと
   「ミュートしたつもりで喋り続ける」）。Meet でミュートを切り替えてから別タブ / 他アプリへ移り、
   出てきた小窓が新しい状態になっていること。**別タブに居るあいだに主催者からミュートされた**
   ときも 1〜2 秒で小窓が OFF になること（背面で Meet の DOM が凍っていない証拠）。
   **小窓側は押しても何も起きない**こと（掴んで動かせる）
