...
- **2026-08-26 v0.5.0 リリース後の実機で判明**: 起動直後の 60 秒間、サイドバーが
  `Connect GitHub`（未設定）に見えていた。
  - **原因は `gh auth token` のタイムアウト 3 秒**。ターミナルからの実測は 20〜30ms なのに、
    **Finder 起動のアプリから初めて子プロセスを exec すると 2.65 秒かかる**
    （dev 版のログで実測: `live_folder.gh elapsedMs=2650`）。常用版ではこれを超えて失敗し、
    「未設定」に落ちていた
  - **診断ログが無かったのが一番の問題**。`readGhToken()` が失敗しても何も残らず、
    「startup の要求が取得も失敗もせずに消えている」ことから推測するしかなかった。
    `live_folder.gh`（found / ok / timedOut / elapsedMs）と `live_folder.no_token`
    （reason / retryInMs）を追加した
  - タイムアウトを **3 秒 → 10 秒**に伸ばした
  - **「本当に未設定」と「解決に失敗した」を分けた**（`ResolvedToken.reason`）。
    後者は 60 秒ではなく **5 秒後に再試行**する。一緒くたにすると、
    呼び出しがたまたま失敗しただけで 60 秒間ずっと未設定に見える
  - 実機で確認: 修正後は `startup` の要求のまま 10 件取得できた
    （`cost: 1` / `remaining: 4975`。**実データでのコストが 1 であることの実測**も取れた）
