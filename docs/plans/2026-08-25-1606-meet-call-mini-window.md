...
| ボタン | **会議へ移動 / マイク / カメラ の 3 つだけ**。**退出も ✕ も置かない**（退出は誤爆で会議を抜ける事故を消すため、✕ は「会議中はいつでも出ている」を崩さないため） |
| テキスト | ドメイン名 + 経過時間（**参加を検知した時刻**からの経過。復元をまたぐと 0 に戻る） |
| 見た目 | 336×60・ボタン 38px・ドメインと経過時間は **2 段**。切れているときは**面を沈めたままアイコンだけ `--nemo-danger` + 斜線**。「会議へ移動」は**ドメイン名ごと 1 つのボタン**（DESIGN.md「会議の小窓」） |
...
**2026-09-08 追記**: 小窓からマイク / カメラを切り替える機能（`call:toggleMic` / `call:toggleCam`・
`buildToggleSource`）は廃止した。押しても切り替わらない・遅れるが直らなかったため。
状態の表示と背面スロットリング解除（表示の鮮度と離脱検知のため）は残っている。
