...
- **⌘⌥↓ / ⌘⌥↑ でサイドバーの行を見た目の順に 1 つずつ渡る**（`select-row-below` / `select-row-above`。
  `settings.json` の `keybindings` で変えられる）。Live Folder → Favorites → ピン留め → 一時タブを横断し、
  閉じたフォルダ・畳んだ PR の小見出しの中は飛ばす。閉じた枠に入るとその場で開く。両端は循環する
...
- **ピン留めと一時タブの境界線をホバーすると「↓ Clear」が出る**。押すと線の直下に赤い「Close all tabs」ボタンが出て、
  それを押すと一時タブを全部閉じる（Enter でも進む。Esc / 外側クリックで何もしない。
  ホバーで出るボタンは誤タップしやすいので 1 回だけ確認を挟む）
  （共有定義は全ウィンドウから消え、`about:blank` などのローカル行はそのウィンドウから消える）。
  見ている最中のタブも音が出ているタブも例外にしない。ピン留め / Favorites は触らない。
  閉じたタブはアーカイブに残るのでライブラリから掘り返せる。別ウィンドウで会議に参加中のタブだけは残る
